...
void FreeIMU::getRawValues(float * raw_values) {
	acc->select();
	raw_values[0] = acc->getX();
	raw_values[1] = acc->getY();
	raw_values[2] = acc->getZ();

	gyro->select();
	raw_values[3] = gyro->getXRate();
	raw_values[4] = gyro->getYRate();
	raw_values[5] = gyro->getZRate();

	mag->select();
	raw_values[6] = mag->getX();
	raw_values[7] = mag->getY();
	raw_values[8] = mag->getZ();

	baro->select();
	raw_values[9] = baro->getTemperature();
	raw_values[10] = baro->getPressure();
}
